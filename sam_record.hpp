...
constexpr bool check(const uint16_t to_check, const uint16_t &f) {
  return to_check & f;
}
inline void set(uint16_t &to_set, const uint16_t f) { to_set |= f; }
inline void unset(uint16_t &to_unset, const uint16_t f) { to_unset &= ~f; }
} // namespace samflags

std::string format_sam_flags(const uint16_t the_flags);
