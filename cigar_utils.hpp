...
inline bool consumes_reference(const char op) {
  return ((op == 'M') || (op == 'D') || (op == 'N') || (op == '=') ||
          (op == 'X'));
}

template <class InItr>
static size_t extract_op_count(InItr &itr, const InItr last) {
  size_t x = 0;
...
size_t cigar_total_ops(InputItr itr, const InputItr last) {
  size_t op_count = 0;
  while (itr != last) {
...
template <class InputItr>
size_t cigar_qseq_ops(InputItr itr, const InputItr last) {
  size_t op_count = 0;
  while (itr != last) {
...
template <class T> size_t cigar_qseq_ops(const T &cigar) {
  return cigar_qseq_ops(std::begin(cigar), std::end(cigar));
}

template <class InputItr>
size_t cigar_rseq_ops(InputItr itr, const InputItr last) {
  size_t op_count = 0;
  while (itr != last) {
    const size_t curr_val = extract_op_count(itr, last);
...
template <class T> constexpr size_t cigar_rseq_ops(const T &cigar) {
  return cigar_rseq_ops(std::begin(cigar), std::end(cigar));
}

template <class InputItr>
void reverse_cigar(InputItr left, const InputItr last) {
...
template <class T> void reverse_cigar(T &cigar) {
  reverse_cigar(std::begin(cigar), std::end(cigar));
}

template <class InputItr>
InputItr truncate_cigar(InputItr left, const InputItr last,
                        const size_t target_ops) {
...
template <class InputItr>
InputItr truncate_cigar_q(InputItr left, const InputItr last,
                          const size_t target_ops) {
...
template <class T> void truncate_cigar_q(T &cigar, const size_t target_ops) {
  /* truncate cigar after specified number of query operations */
...
template <class InputItr>
InputItr truncate_cigar_r(InputItr left, const InputItr last,
                          const size_t target_ops) {
...
template <class T> void truncate_cigar_r(T &cigar, const size_t target_ops) {
  /* truncate cigar after specified number of reference operations */
  auto c_beg(std::begin(cigar));
...
template <class InputItr>
InputItr merge_equal_neighbor_cigar_ops(InputItr rd_itr, const InputItr last) {
...
template <class T> void merge_equal_neighbor_cigar_ops(T &cigar) {
  /* renders a cigar valid by merging consecutive identical operations */
  auto c_beg(std::begin(cigar));
...
template <class InputItr>
void internal_S_to_M(InputItr rd_itr, const InputItr last) {
...
template <class T> void internal_S_to_M(T &cigar) {
  /* converts internal soft-clip (S) symbols into match/mismatch (M) symbols */
  internal_S_to_M(std::begin(cigar), std::end(cigar));
}

template <class T> void terminal_S_to_M(T &cigar) {
...
template <class InputItr>
void initial_S_to_M(InputItr rd_itr, const InputItr last) {
...
template <class T> void initial_S_to_M(T &cigar) {
  /* converts an initial soft-clip symbol to a match/mismatch (M) symbol */
  initial_S_to_M(std::begin(cigar), std::end(cigar));
}

template <class InputItr>
size_t get_soft_clip_size(InputItr rd_itr, const InputItr last) {
...
template <class T> constexpr size_t get_soft_clip_size(T &cigar) {
  /* determine the total size of the soft-clips at both ends */
  return get_soft_clip_size(std::begin(cigar), std::end(cigar));
}

template <class InputItr>
size_t get_soft_clip_size_start(InputItr rd_itr, const InputItr last) {
  /* determine the size of the soft-clip at the start of cigar */
  const size_t curr_op_count = extract_op_count(rd_itr, last);
  return (*rd_itr == 'S') ? curr_op_count : 0;
}

template <class T> constexpr size_t get_soft_clip_size_start(T &cigar) {
  /* determine the size of the soft-clip at the start of cigar */
...
void compress_cigar(T1 c_itr, const T1 c_end, T2 &cigar) {
...
template <class T1, class T2>
void uncompress_cigar(T1 c_itr, const T1 c_end, T2 &cigar) {
...
void apply_cigar(const std::string &cigar, std::string &to_inflate,
